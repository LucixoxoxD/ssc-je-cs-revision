  ...
  "topics": [
    {
      ...
      "keyPoints": [
        ...
        "Scheduler Hierarchy: Long-Term Scheduler (Job Scheduler) loads jobs from disk spool to RAM and controls the Degree of Multiprogramming; Short-Term Scheduler (CPU Scheduler) selects ready processes for CPU execution at high frequency ($10\\text{ to }100\\text{ ms}$); Medium-Term Scheduler (Swapper) swaps processes between RAM and swap space to manage memory pressure.",
        "Preemptive vs Non-Preemptive Scheduling: Non-preemptive scheduling occurs only on transitions: $\\text{Running} \\to \\text{Waiting}$ (I/O request) or $\\text{Running} \\to \\text{Terminated}$. Transitions: $\\text{Running} \\to \\text{Ready}$ (timer interrupt) or $\\text{Waiting} \\to \\text{Ready}$ (I/O complete) represent preemptive scheduling."
      ],
      "tables": [
        {
        ...
        },
        {
          ...
          "rows": [
            ...
            ["Short-Term (CPU Scheduler)", "Ready → Running", "Extremely frequent ($10\\text{ to }100\\text{ ms}$)", "Maximizes CPU utilization, throughput, and fairness"],
      ...
      "examples": [
        {
          ...
          "steps": [
            ...
            "Step 2: Recall the formula for total processes produced by $n$ consecutive fork() calls: $N_{\\text{total}} = 2^n = 2^3 = 8$ processes (this count includes the original parent process).",
            "Step 3: Calculate the number of new child processes: $N_{\\text{child}} = 2^n - 1 = 2^3 - 1 = 8 - 1 = 7$ child processes.",
        ...
        {
          "question": "A uniprocessor time-sharing operating system assigns a time quantum of $18\\text{ ms}$ to each process. Every context switch between two processes takes $2\\text{ ms}$ of CPU time. What is the effective CPU utilization of this system, and how much percentage is lost to context switching overhead?",
          "steps": [
            "Step 1: Determine the useful CPU execution time per scheduling cycle: $T_{\\text{useful}} = \\text{Time Quantum} = 18\\text{ ms}$.",
            "Step 2: Determine total cycle duration: $T_{\\text{cycle}} = T_{\\text{useful}} + T_{\\text{switch}} = 18\\text{ ms} + 2\\text{ ms} = 20\\text{ ms}$.",
            "Step 3: Apply the CPU utilization formula: $$\\text{CPU Utilization} = \\frac{T_{\\text{useful}}}{T_{\\text{cycle}}} \\times 100\\%$$",
            "Step 4: Compute: $$\\frac{18}{20} \\times 100\\% = 0.90 \\times 100\\% = 90\\%$$",
            "Step 5: Compute overhead percentage: $100\\% - 90\\% = 10\\%$ lost to context switching."
          ],
      ...
      "traps": [
        ...
        "Trap 4 (Preemption Transition Ambiguity): The transition from $\\text{Waiting} \\to \\text{Ready}$ (e.g., I/O finished) is NOT preemption! Preemption strictly means taking the CPU away from an actively Running process against its will ($\\text{Running} \\to \\text{Ready}$)."
      ],
      "practice": [
        {
